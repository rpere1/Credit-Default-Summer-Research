...
temp%>%filter(inst<100)%>%group_by(loan_outcome)%>%summarize(inst = mean(inst), n = n())
#corrplot(cor(data1[getNumericColumns(data1)],use="na.or.complete"))
#create a installment variable variable 
#load packages
library(dplyr)
library(tidyverse)
library(caret)
...
#combine datasets until split later 
...
temp = dplyr::select(train1, -c(some_college, hs, poverty_rate, growth, loan_outcome,grade_A, grade_B, grade_D, grade_E, fico_range_high))
temp1 = dplyr::select(test1, -c(some_college, hs, poverty_rate, growth, loan_outcome,grade_A, grade_B, grade_D, grade_E, fico_range_high))

#Logistic Regression
ctrl <- trainControl(method = "cv",summaryFunction = twoClassSummary,classProbs = TRUE,number = 5)

glmnGrid = expand.grid(.alpha = seq(0, 1, length = 10), .lambda = 0)
...
#random Forest, ntree = 375 is when improvement slows down dramatically
...
set.seed(123)
rf_tun2 <- train(loan_outcome~.,
                    data = train1,
                    method = "rf",
                    ntree = 500,
                    trControl = trControl, 
                 tuneGrid = data.frame(mtry = 5))
rf_tune <- randomForest(loan_outcome ~ .,data=train1,  ntree = 375, do.trace=F)
rf_res = predict(rf_tune, test1[-35], type = 'prob')
rf_res <- as.data.frame(rf_res)
rf_res <- rf_res$`default`

plot(roc_rf,legacy.axes = TRUE,col="blue",main="ROC compare")
plot(roc_rf2,legacy.axes = TRUE,col="purple",add=TRUE)
...
legend("bottomright",legend=c("RF","RF2"),fill=c("blue","purple"))
#gbm model (tuning process not shown), ROC = 0.7372
#create weights 
...
write.csv(ensemble_res, "ensemble_res")
...
#creat a subset of variables that were significant based on the alsso regression 
lasso_sub = dplyr::select(train1, c(loan_outcome, installment, emp_length, dti, fico_range_high, pub_rec, revol_bal, revol_util, open_acc_6m, open_il_24m, open_rv_24m, max_bal_bc, all_util, total_cu_tl, num_rev_tl_bal_gt_0, num_sats, num_tl_120dpd_2m, num_tl_90g_dpd_24m, open_acc_ratio, grade_A, grade_B, grade_C, grade_D, grade_E, grade_F, grade_G, home_ownership_MORTGAGE, home_ownership_RENT, verification_status_Verified, initial_list_status_w, term_36_months, verification_status_not_verified, term_60_months))
fs_glm = glm(loan_outcome~., data=lasso_sub,family = "binomial")
